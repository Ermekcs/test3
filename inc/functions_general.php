...
function online_users($view_moder = 1)
{
	global $database;

  $online_array = NULL;

  $moder_where = ($view_moder) ? 'user_moderator_mode = 0' : 'visitor_user_moderator=0 || (visitor_user_moderator = 1 && visitor_user_moderator_mode = 1)';
  ...
  if( !is_array($online_array) )
  {
    ...
    $sql = "SELECT visitor_user_id AS user_id,
    			   visitor_user_username AS user_username,
    			   visitor_user_moderator AS user_moderator,
    			   visitor_user_moderator_mode AS user_moderator_mode,
    			   visitor_user_displayname AS user_displayname
    			   FROM se_visitors
    			   WHERE visitor_invisible=0 && visitor_lastactive>'{$online_time}' && {$moder_where}
    			   ORDER BY visitor_lastactive DESC LIMIT 2000";
    ...
    while( $online_user_info = $database->database_fetch_assoc($resource) )
    {
      ...
      if( !empty($online_user_info['user_id']) )
      {
        ...
        $online_user->user_info['user_moderator']   = $online_user_info['user_moderator'];
        $online_user->user_info['user_moderator_mode'] = $online_user_info['user_moderator_mode'];
...
function online_moderators(){

	global $database;

	$moderators_array = NULL;

  // CACHING
  $cache_object = SECache::getInstance('serial');
  if( is_object($cache_object) )
  {
    $moderators_array = $cache_object->get('online_users');
  }

  if( !is_array($moderators_array) )
  {
    $total_visitors = 0;
    $onlineusers_array = array();
    $onlineusers_usernames = array();
    ...
    $sql = "SELECT visitor_user_id AS user_id,
    			   ...
    			   visitor_user_displayname AS user_displayname
    		FROM se_visitors
    		LEFT JOIN se_users ON (se_users.user_id = se_visitors.visitor_user_id)
    		WHERE se_users.user_level_id <> ".OBSERVER_LEVEL_ID." AND visitor_invisible=0 && visitor_lastactive>'{$online_time}' && visitor_user_moderator=1 && visitor_user_moderator_mode=0
    		ORDER BY visitor_lastactive DESC LIMIT 2000";
    $resource = $database->database_query($sql);
    ...
    $moderators_array = array($onlineusers_array, $total_visitors, $onlineusers_usernames);

    // CACHE
    if( is_object($cache_object) )
    {
      $cache_object->store($moderators_array, 'online_users');
    }
  }
...
function online_observers(){
  ...
  if( is_object($cache_object) )
  {
    $moderators_array = $cache_object->get('online_observers');
  }
  ...
  if( !is_array($moderators_array) )
  {
    ...
    $sql = "SELECT visitor_user_id AS user_id,
    		...
    		FROM se_visitors
    		LEFT JOIN se_users ON (se_users.user_id = se_visitors.visitor_user_id)
    		WHERE visitor_invisible=0 && visitor_lastactive>'{$online_time}' && visitor_user_moderator=1 && visitor_user_moderator_mode=0 AND se_users.user_level_id = ".OBSERVER_LEVEL_ID."
    		ORDER BY visitor_lastactive DESC LIMIT 2000";
    $resource = $database->database_query($sql);
    ...
    while( $online_user_info = $database->database_fetch_assoc($resource) )
    {
      ...
      if( !empty($online_user_info['user_id']) )
      {
        ...
        $online_user->user_info['user_moderator']   = $online_user_info['user_moderator'];
        $online_user->user_info['user_moderator_mode'] = $online_user_info['user_moderator_mode'];
        $online_user->user_info['user_level_id'] = $online_user_info['user_level_id'];
        $online_user->user_displayname              = $online_user_info['user_displayname'];
    ...
    if( is_object($cache_object) )
    {
      $cache_object->store($moderators_array, 'online_observers');
    }
  }
...
{
  global $setting, $database, $user, $he_is_moderator;
  ...
  if( !is_array($signups) )
  {
    ...
    if ($he_is_moderator){
        ...
        $subnet_where = "AND `user_subnet_id` = {$subnet_id}";
    }

    $sql = "SELECT user_id, user_username, user_moderator, user_moderator_mode, user_fname, user_lname, user_photo, user_level_id FROM se_users WHERE user_verified='1' AND user_enabled='1' AND user_search='1' {$subnet_where} ORDER BY user_signupdate DESC LIMIT 20";
    $resource = $database->database_query($sql);
    ...
    while( $user_info = $database->database_fetch_assoc($resource) )
    {
      ...
      $signup_user->user_info['user_moderator'] = $user_info['user_moderator'];
      $signup_user->user_info['user_moderator_mode'] = $user_info['user_moderator_mode'];
      ...
      $signup_user->user_info['user_level_id'] = $user_info['user_level_id'];
...
function recent_logins()
{
  ...
  if( !is_array($logins) )
  {
    $sql = "SELECT user_id, user_username, user_moderator, user_moderator_mode, user_fname, user_lname, user_photo, user_level_id FROM se_users WHERE user_photo<>'' AND user_search='1' AND user_moderator=0 || (user_moderator = 1 && user_moderator_mode = 1) ORDER BY user_lastlogindate DESC LIMIT 20";
    $resource = $database->database_query($sql);
    ...
    while( $user_info = $database->database_fetch_assoc($resource) )
    {
      ...
      $login_user->user_info['user_moderator'] = $user_info['user_moderator'];
      $login_user->user_info['user_moderator_mode'] = $user_info['user_moderator_mode'];
      ...
      $login_user->user_info['user_level_id'] = $user_info['user_level_id'];
...
function popular_users()
{
  ...
  {
    $sql = "SELECT count(se_friends.friend_user_id2) AS num_friends, se_users.user_id, se_users.user_username, se_users.user_moderator,
    	" . " se_users.user_moderator_mode, se_users.user_fname, se_users.user_lname, se_users.user_photo, user_level_id
    	" . " FROM se_friends LEFT JOIN se_users ON se_friends.friend_user_id1=se_users.user_id
		" . " LEFT JOIN se_levels ON se_users.user_level_id=se_levels.level_id
    	" . " WHERE se_friends.friend_status='1' AND se_users.user_search='1' AND se_levels.level_moderator!=1
    	" . " GROUP BY se_users.user_id ORDER BY num_friends DESC LIMIT 20";
    ...
    while( $user_info = $database->database_fetch_assoc($resource) )
    {
      ...
      $popular_user->user_info['user_moderator'] = $user_info['user_moderator'];
      $popular_user->user_info['user_moderator_mode'] = $user_info['user_moderator_mode'];
      ...
      $popular_user->user_info['user_level_id'] = $user_info['user_level_id'];
...
function he_extract_youtube_code($video_url)
{
...
function he_new_forum_youtube_video( $id, $youtube_url )
{
	if (!$id)
		return false;

	$youtube_code = he_extract_youtube_code($youtube_url);
	he_video_update_youtube_thumb($youtube_code);

	$sql = he_database::placeholder("UPDATE se_forumposts SET forumpost_video_code = '?' WHERE forumpost_id = ?", $youtube_code, $id);

	return (bool)he_database::query($sql);
}

function he_frontend_show_youtube_video( $params )
{
	global $smarty;

	$post_id = $params['post_id'];
	$youtube_code = get_forum_post_video( $post_id );

	$smarty->assign('video_youtube_code', $youtube_code);
	return $smarty->fetch('forum_youtube_video.tpl', null, null, false);
}


function get_forum_post_video( $post_id )
{
	if (!$post_id)
		return false;

	$sql = he_database::placeholder("SELECT forumpost_video_code FROM se_forumposts WHERE forumpost_id = ?", $post_id);
	return he_database::fetch_field($sql);
}

function user_directory( $user_id )
{
	if (!$user_id)
		return false;

	$main_dir = ((int)($user_id / 1000)) * 1000;

	return "./uploads_user/".$main_dir."/".$user_id."/";
}

function he_video_update_youtube_thumb( $video_youtube_code )
{
	global $user;

	if (!$video_youtube_code)
		return false;
	...
	$video_directory = user_directory( $user->user_info['user_id'] );

	$thumb_dimensions = @getimagesize($thumb_source_path);
	$thumb_width = $thumb_dimensions[0];
	$thumb_height = $thumb_dimensions[1];

	$destination = $video_directory . $video_youtube_code . '_thumb.jpg';
...
function long_inactivity(){
    ...
    $max = $setting['setting_inactivity_max']*3600*24;
    $period = $setting['setting_inactivity_period']*3600*24;
    $last = $setting['setting_inactivity_last'];
    $enabled = $setting['setting_inactivity_enabled'];

    if (!$enabled || $last == $date){ return; }

    $start = $time-$max;
    $end = $start-$period;

    $sql = he_database::placeholder("SELECT `user_id` FROM `se_users`
        WHERE `user_lastlogindate` < ? AND `user_lastlogindate` > ?
            AND `user_enabled` = 1", $start, $end);

    $users = he_database::fetch_column($sql);

    foreach ($users as $user_id){

        $owner = new se_user(array($user_id));
        ...
        send_systememail(
            ...
            array(
                ...
                se_datetime::cdate($setting['setting_dateformat'], $owner->user_info['user_lastlogindate']),
                $url->url_base
            )
        );
    }

    he_database::query("UPDATE `se_settings` SET `setting_inactivity_last` = '{$date}'");

}


function explaining_disable($owner, $text){
        ...
        he_database::query("UPDATE `se_users` SET `user_is_notify_explanation_disable` = 1
            WHERE `user_id` = {$owner->user_info['user_id']}");
}


function check_recapcha() {

	global $setting, $_SERVER, $_REQUEST;

	$response = file_get_contents('http://www.google.com/recaptcha/api/verify?privatekey=' . urlencode($setting['setting_recaptcha_privatekey']) . '&remoteip=' . urlencode($_SERVER['REMOTE_IP']) . '&challenge=' . urlencode($_REQUEST['recaptcha_challenge_field']) . '&response=' . urlencode($_REQUEST['recaptcha_response_field']));
	$response = explode("\n", $response);
...
function gen_admin_pass($len = 14) {
    $pass = '';
    $rnd = 0;
    $c = '';
